...
def qrheston(s0, v0, a, b, c, mu, theta, lam, nu, H, rho, m, n, T, device=None, seed=123):
    """
        Inputs:
            s0: float tensor of shape (m,1), initial prices
            v0: float tensor of shape (m,1), initial vol
            a: float tensor of shape (m)
            b: float tensor of shape (m)
            c: float tensor of shape (m)
        ...
        Outputs:
            dict of 
                torch float tensor of shape (m,n): m quadratic rough Heston trajectories of the price s, of n points on an uniformly spaced grid on [0,T]
    """
    if seed is not None:
    ...
    z = torch.zeross(m, n, dtype=torch.float, device=device)
    for i in range(1,n):
        z[:,i] = integ(z, i, delta_t, H, lam, theta, device) + lam*nu*integ2(z, i, delta_t, H, gaussian_b, device)
        v[:,i] = a*(z[:,i]-b)**2 + c
        log_s[:,i] = log_s[:,i-1] + torch.sqrt(v[:,i])*gaussian_w[:,i-1]
    rst = {
